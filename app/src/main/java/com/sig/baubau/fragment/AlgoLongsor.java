...
import com.sig.baubau.model.aMatrik;
...
public class AlgoLongsor extends Fragment {
    String ambilData = "";
    String teks = "";
    LongsorDB db;
    int iterasi = 0;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_algo_longsor, container, false);

        db = new LongsorDB(v.getContext());

        RecyclerView rvMatriks = v.findViewById(R.id.rvMarik);
        aMatrik adapter1 = new aMatrik(getMatriks());
        ...
        if (cursor.moveToFirst()) {
            do {
                String ex = cursor.getString(cursor.getColumnIndex(LongsorDB.row_kdKec));
                kumpulKode.add(ex);
                float a = Float.parseFloat(cursor.getString(cursor.getColumnIndex(LongsorDB.row_tidak)));
                float b = Float.parseFloat(cursor.getString(cursor.getColumnIndex(LongsorDB.row_rendah)));
                float c = Float.parseFloat(cursor.getString(cursor.getColumnIndex(LongsorDB.row_sedang)));
                kode.add(ex);
                tidak.add(a);
                rendah.add(b);
                sedang.add(c);
        ...
        teks+= "ITERASI 3\n";
        kMedoid(matriks, k3);
        finalkMedoid(matriks, k2);

        TextView tampil = v.findViewById(R.id.tvAlgoalgo1);
        tampil.setText(teks);
    ...
    private List getMatriks(){
        Cursor cursor = db.allData();
        List<cMatrik> data = new ArrayList<>();
        if (cursor.moveToFirst()){
            do {
                data.add(new cMatrik(cursor.getString(cursor.getColumnIndex(LongsorDB.row_kdKec)),
                        cursor.getString(cursor.getColumnIndex(LongsorDB.row_tidak)),
                        cursor.getString(cursor.getColumnIndex(LongsorDB.row_rendah)),
                        cursor.getString(cursor.getColumnIndex(LongsorDB.row_sedang))));
            } while (cursor.moveToNext());
        }
        return data;
    }
